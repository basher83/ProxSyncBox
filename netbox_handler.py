...
def get_or_create_cluster(nb: pynetbox.api, cluster_name: str, cluster_type_name: str) -> Optional[Any]:
    if not nb or not cluster_type_name: return None # Added check for cluster_type_name
...
def get_or_create_cluster_type(nb: pynetbox.api, cluster_type_name: str) -> Optional[Any]:
    ...
    logger.info(f"Cluster type '{cluster_type_name}' not found. Creating...")
    try: # "Creating..."
        return nb.virtualization.cluster_types.create(name=cluster_type_name, slug=cluster_type_name.lower().replace(" ", "-")) # Create slug from name
    except pynetbox.core.query.RequestError as e:
        logger.error(f"Error creating cluster type '{cluster_type_name}': {e.error if hasattr(e, 'error') else e}")
        return None
...
def get_or_create_netbox_platform(nb: pynetbox.api, platform_name: str) -> Optional[int]:
    ...
    try:
        ...
        if "unique constraint" in error_message.lower() or "already exists" in error_message.lower():
            ...
            else: logger.error(f"Error creating/fetching platform '{platform_name}' post-conflict: {error_message}")
        else: logger.error(f"Error creating platform '{platform_name}': {error_message}")
        return None
...
def get_or_create_and_assign_netbox_mac_address(
    ...
    try:
        ...
        logger.debug(f"Searching for MAC '{mac_str_upper}'. Filter (by mac_address only) returned {len(mac_objects_from_filter)} objects.")
        ...
        if assign_to_interface_id and assigned_object_type:
            ...
            for obj_summary_idx, obj_summary in enumerate(mac_objects_from_filter):
                logger.debug(f"  Fetching full MAC Obj {obj_summary_idx+1}/{len(mac_objects_from_filter)} (Summary ID: {obj_summary.id}) for assignment check to Interface ID {assign_to_interface_id}")
                try:
                ...
                if not (hasattr(obj, 'mac_address') and obj.mac_address and obj.mac_address.upper() == mac_str_upper):
                    logger.warning(f"    FULL MAC Object ID {obj.id} has mismatching mac_address '{getattr(obj, 'mac_address', 'N/A')}' (expected '{mac_str_upper}'). Skipping.")
                    continue

                # Now, use the direct assigned_object_id and assigned_object_type fields from the full 'obj'
                if hasattr(obj, 'assigned_object_id') and obj.assigned_object_id is not None and \
                   hasattr(obj, 'assigned_object_type') and obj.assigned_object_type is not None:
                    ...
                    logger.debug(f"    FULL MAC Obj ID {obj.id} direct assignment fields: assigned_object_id={assigned_obj_id}, assigned_object_type='{assigned_obj_content_type_from_mac_obj}'")
                    if target_assigned_object_type_lower: # Ensure we have a target type to compare against
                        logger.debug(f"    Comparing with target: interface_id={assign_to_interface_id}, type='{target_assigned_object_type_lower}'")

                        if assigned_obj_id == assign_to_interface_id and assigned_obj_content_type_from_mac_obj == target_assigned_object_type_lower:
                            logger.info(f"MAC Address '{mac_str_upper}' (ID: {obj.id}) is already correctly assigned to interface ID {assign_to_interface_id} (Type: {assigned_object_type}) using direct fields from full object. Reusing.")
                            return obj 
                    else:
                        logger.debug(f"    FULL MAC Obj ID {obj.id}: Target assigned_object_type is not specified for comparison, cannot confirm assignment.")
                else:
                    logger.debug(f"    FULL MAC Obj ID {obj.id}: Missing assigned_object_id or assigned_object_type on full object.")

            # If the loop finishes, no correctly assigned MAC object was found.
            logger.debug(f"No existing MAC Address object for '{mac_str_upper}' found already assigned to interface ID {assign_to_interface_id} after checking {len(mac_objects_from_filter)} full objects.")
        ...
        log_interface_info = f"interface ID {assign_to_interface_id} (Type: {assigned_object_type})" if assign_to_interface_id else "an unassigned context"
        logger.info(f"MAC Address '{mac_str_upper}' is not currently assigned to {log_interface_info}. Creating a new MACAddress object for this assignment.")
        ...
        try:
            ...
            if not created_mac_obj:
                logger.error(f"Failed to create new MAC Address object for '{mac_str_upper}'. Create call returned None/False.")
                return None

            logger.info(f"Successfully created new MAC Address object ID: {created_mac_obj.id} for MAC: {mac_str_upper}.")
            ...
            if assign_to_interface_id and assigned_object_type:
                ...
                logger.info(f"Assigning newly created MAC {mac_str_upper} (ID: {created_mac_obj.id}) to interface ID: {assign_to_interface_id} (Type: {obj_type_to_assign})")
                update_payload = {
                ...
                try:
                    if not created_mac_obj.update(update_payload):
                        logger.warning(f"Failed to assign newly created MAC {mac_str_upper} (ID: {created_mac_obj.id}) to interface {assign_to_interface_id} (update() returned False).")
                    ...
                    logger.error(f"Error assigning newly created MAC {mac_str_upper} (ID: {created_mac_obj.id}) to interface {assign_to_interface_id}: {err_msg_assign}")
            ...
            logger.error(f"NetBox API error during CREATION of new MACAddress object for '{mac_str_upper}': {error_str}")
            ...
            if "unique constraint" in error_str or "already exists" in error_str or "mac address with this address already exists" in error_str:
                logger.warning(f"Creation of MAC '{mac_str_upper}' failed due to uniqueness. Attempting to re-fetch by MAC string only.")
                ...
                if mac_objects_retry_filter:
                    ...
                    if len(mac_objects_retry_filter) > 1:
                         logger.warning(f"MAC Address '{mac_str_upper}' has multiple objects in NetBox ({len(mac_objects_retry_filter)} found on re-fetch). Using the first one (ID: {found_mac_obj_on_retry.id}).")
                    logger.info(f"Re-fetch successful after unique constraint error. Found existing MAC '{mac_str_upper}' with ID: {found_mac_obj_on_retry.id}.")
                    ...
                    logger.error(f"MAC '{mac_str_upper}' exists (ID: {found_mac_obj_on_retry.id}) but is not assigned to interface ID {assign_to_interface_id} (as per initial check). Cannot create a new one due to uniqueness. Skipping MAC assignment for this interface.")
                    return None # Indicate failure to get/create/assign the MAC object for this interface.
                else:
                    logger.error(f"Re-fetch for MAC '{mac_str_upper}' failed to find an exact match even after unique constraint error on create. NetBox data might be inconsistent or filter is unreliable. Skipping MAC assignment for this interface.")
                    return None
            else: # Other creation error not related to uniqueness
                return None
        except Exception as e_generic_create:
            logger.error(f"Unexpected error during MAC creation block for '{mac_str_upper}': {e_generic_create}", exc_info=True)
            return None

    except Exception as e_outer:
        logger.error(f"Unexpected error in get_or_create_and_assign_netbox_mac_address (outer block) for '{mac_str_upper}': {e_outer}", exc_info=True)
        return None

def get_or_create_site(nb: pynetbox.api, site_name: str, site_slug: Optional[str] = None) -> Optional[pynetbox.core.response.Record]:
...
def get_or_create_manufacturer(nb: pynetbox.api, manu_name: str, manu_slug: Optional[str] = None) -> Optional[pynetbox.core.response.Record]:
...
def get_or_create_device_type(nb: pynetbox.api, model_name: str, manufacturer_id: int, dt_slug: Optional[str] = None, u_height: int = 1) -> Optional[pynetbox.core.response.Record]:
    ...
    logger.info(f"Device Type '{model_name}' (Manufacturer ID: {manufacturer_id}) not found. Creating with slug '{slug}'...") # "Creating..."
    try:
...
def get_or_create_device_role(nb: pynetbox.api, role_name: str, role_slug: Optional[str] = None, color_hex: str = "00bcd4") -> Optional[pynetbox.core.response.Record]:
...
def get_or_create_device_interface(
    ...
    if existing_iface:
        ...
        if update_payload: # If there are changes, update the interface
            logger.info(f"Updating interface '{iface_name}' (ID: {existing_iface.id}) on device ID {device_id}. Payload: {update_payload}")
            
            # Log the state before the update attempt
            logger.debug(f"Interface '{iface_name}' (ID: {existing_iface.id}) before update (excluding MAC string): Type='{existing_iface.type}', Enabled={existing_iface.enabled}, Desc='{existing_iface.description}', CF={existing_iface.custom_fields}")
            ...
            try: 
                ...
                try: # Inner try for re-fetching
                    ...
                    if refetched_iface: logger.debug(f"Interface '{iface_name}' (ID: {existing_iface.id}) after update (excluding MAC string): Type='{refetched_iface.type}', Enabled={refetched_iface.enabled}, Desc='{existing_iface.description}', CF={existing_iface.custom_fields}. MAC string should be set by primary_mac_address link.")
                    else: logger.warning(f"Could not re-fetch interface {existing_iface.id} after update.")
                except pynetbox.core.query.RequestError as e_refetch: logger.warning(f"Error re-fetching interface {existing_iface.id} after update: {e_refetch}")
                ...
                logger.error(f"Error updating interface '{iface_name}' (ID: {existing_iface.id}) for device ID {device_id}: {error_message}")
    ...
    try:
        ...
        logger.error(f"Error creating interface '{iface_name}' for device ID {device_id}: {e.error if hasattr(e, 'error') else e}")
        return None